...
func TestClientGenerateHandlesNon2xx(t *testing.T) {
	...
	assertGeminiRequestDiagnostics(t, response.RequestMessages)
}
...
func assertGeminiRequestDiagnostics(t *testing.T, messages []outbound.ModelRequestMessage) {
	t.Helper()

	if len(messages) != 2 {
		t.Fatalf("len(RequestMessages) = %d, want provider-facing diagnostics", len(messages))
	}
	if messages[0].Role != "system_instruction" ||
