...
The default sandbox provider is noop. The Docker sandbox provider is dev-only and may invoke the Docker CLI with `os/exec`, but it must never execute requested commands directly on the host. `sandbox_exec` must only be advertised when structured workspace input/work paths exist and the sandbox reports `SupportsCommands`.
