...
AgentPool is a self-hosted runtime server for team AI agent runs. It accepts run submissions, manages run lifecycle, dispatches runs to workers, executes through an application-owned agent runner, prepares execution through an abstract `SandboxProvider`, calls model providers through an abstract `ModelClient`, and publishes lifecycle events through an abstract `EventPublisher`.
...
Agent behavior belongs in `internal/application/agent`, including agent loop mechanics, provider-neutral action parsing, and tool orchestration. Model provider integrations belong in `internal/infrastructure/llm`. Provider packages must implement `internal/application/port/outbound.ModelClient`; they must not contain agent loops, planning, tool orchestration, evaluation logic, or workflow state transitions. Tool execution implementations belong under `internal/infrastructure/tool/*` and must implement `internal/application/port/outbound.ToolRunner`. Workspace tools must stay read-only, confined to `outbound.Sandbox.WorkspacePath`, and reject path traversal, absolute paths, symlink escapes, binary files, and oversized reads. Do not add arbitrary shell, unrestricted filesystem, network, Git mutation, package install, Docker, or command-execution tools without an intentionally designed sandbox and policy gate. Do not add provider-specific SDK types, JSON wire structs, or provider-specific concepts to application ports.

Workspace source selection belongs to run task input and the `Run` aggregate task spec. Workspace provider implementations live under `internal/infrastructure/workspace/*` and must implement `internal/application/port/outbound.WorkspaceProvider`. Workspace tools must only be advertised when `outbound.Sandbox.WorkspacePath` is available. `AGENTPOOL_WORKSPACE_PATH` is only the configured local workspace source; it must not implicitly grant workspace access to every run.

Legacy Git provider experiments may remain under `internal/infrastructure/git/*`, but current workspace source behavior belongs in `internal/infrastructure/workspace/*`. Local workspace providers must not mutate files, run git commands, fetch from the network, or implement real clone behavior. A real git clone provider is a future feature.
