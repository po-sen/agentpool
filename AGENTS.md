...
Do not add persistent workspace storage, archive materialization, workspace diffing, file mutation, shell execution, or worker-local path workspace providers without explicit design review. Future workspace source implementations should be explicit product-authorized input sources, not product ACL logic inside AgentPool. Local path access is not core behavior.
