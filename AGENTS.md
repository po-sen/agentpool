...
Local workspace source behavior belongs in `internal/infrastructure/git/local`. It is a dev/local source provider that returns an existing workspace path through `outbound.GitCheckout.Path`; it must not mutate files, run git commands, fetch from the network, or implement real clone behavior. A real git clone provider is a future feature.
