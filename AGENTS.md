...
For deterministic or verifiable tasks, the agent prompt should prefer `sandbox_exec` when it is available before giving a final exact answer. This is a prompt behavior rule, not a new model-facing tool. Do not add a classifier or hard-coded semantic detector for this unless it is explicitly designed later.
